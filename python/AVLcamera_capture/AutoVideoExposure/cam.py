...
class Camera(object):
    def __init__(self, device_name):
        ...
        #brightness
        qctrl.id = v4l2.V4L2_CID_BRIGHTNESS #v4l2.V4L2_CID_EXPOSURE_AUTO # V4L2_CID_EXPOSURE_ABSOLUTE #V4L2_CID_AUTOGAIN
        

        extCtrls.controls.contents = extCtrl
        extCtrls.count = 1
        extCtrls.ctrl_class = v4l2.V4L2_CTRL_CLASS_CAMERA

 #V4L2_CID_BRIGHTNESS
        ioctl(self.fd, v4l2.VIDIOC_QUERYCTRL, qctrl)
        ctrl.id = qctrl.id
        ioctl(self.fd, v4l2.VIDIOC_G_EXT_CTRLS, ctrl)
        try:
            #ioctl(self.fd, v4l2.VIDIOC_QUERYCTRL, ctrl)
            #ctrl.id = qctrl.id
            print( "id : ", qctrl.id," | name : ", qctrl.name, " | flags : ", qctrl.flags, " | minimum : ", qctrl.minimum, " | maximum : ",qctrl.maximum, " step : ", qctrl.step," | type : ", qctrl.type, " | default : ", qctrl.default_value)
        ...
        (idx, v, mn, mx) = self.controls["brightness"]

        extCtrl = v4l2.v4l2_ext_control()
        extCtrls = v4l2.v4l2_ext_controls()
        extCtrl.id = idx
        extCtrl.value = val
        #extCtrls.controls = ctypes.POINTER(extCtrl)
        extCtrls.controls.contents = extCtrl
        extCtrls.count = 1
        extCtrls.ctrl_class = v4l2.V4L2_CTRL_CLASS_CAMERA

        #ioctl(self.fd, v4l2.VIDIOC_S_EXT_CTRLS, extCtrls)
        try:
            #ioctl(self.fd, v4l2.VIDIOC_S_EXT_CTRLS, extCtrls)
            self.controls["brightness"] = (idx, extCtrl.value, mn, mx)
